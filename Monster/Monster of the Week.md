This is the repo for a Monster of the Week campaign

I'm keeping ongoing story arcs, characters and finished sessions together with ideas for future sessions. I'm also including resources like potential names, locations and easter eggs. The [[Keeper Guide]] gives quick notes for how to run a session.

Note: the notes and session names are basically huge spoilers for any players
# Sessions
Every week, approximately, is another relatively self-contained adventure, with a script using the [[Session x.y template|template schema]] outline.
## upcoming
The shortlist of under-development or ready adventures is here
* [[Session 0.0 Angels with one wing]]: a Los Angeles confectionary shop and distributor is creating magical treats, made from vivisected supernatural entities, that temporarily give the people who eat them an aspect of supernatural essence. An accidental delivery to a children's party provides the heroes the first lead 
## concepts
A running scratch list of ideas is here.
* A demonic or mimic-type of creature creates an intermittent new Metro line that eats(?) people. Call it the X Line

# Characters
* [[Player template schema|player schema]]
* [[NPC template schema|npc schema]]
* [[Location template schema|location schema]]
## Players

## Known NPCs
* [[Mariano Williamson]]

# Schemas
* [[Location template schema]]
* [[Player template schema]]
* [[Session x.y template]]
* [[Monster template schema]]
* [[NPC template schema]]

# Resources

* https://evilhat.com/product/monster-of-the-week/
* https://en.wikipedia.org/wiki/Monster_of_the_Week
* https://blakewatson.com/journal/monster-of-the-week-game-introduction-and-rules-summary/
